                    ...
                    <form method="post" action="{{ route('testimonials.update', $testimonial->id) }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
                        ...
                        <div class="card ">
                            ...
                            <div class="card-body ">
                                ...
                                <div class="row">
                                    <label class="col-sm-2 col-form-label">{{ __('Client Designation') }}</label>
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('client_designation') ? ' has-danger' : '' }}">
                                            <textarea
                                                class="form-control{{ $errors->has('client_designation') ? ' is-invalid' : '' }}"
                                                name="client_designation" id="client_designation" placeholder="{{ __('Client Designation') }}"
                                                required>{{ old('client_designation', $testimonial->client_designation) }}</textarea>
                                            @if ($errors->has('client_designation'))
                                                <span id="phone_no-error" class="error text-danger"
                                                    for="input-phone_no">{{ $errors->first('client_designation') }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-7">
                                        ...
                                        <div class="fileinput @if(!$testimonial->client_image) fileinput-new @else fileinput-exists @endif text-center" data-provides="fileinput">
                                            ...
                                            <div class="fileinput-preview fileinput-exists thumbnail">
                                                    <img src="{{ asset('web/images/avatar/' . $testimonial->client_image ) }}" />
                                            </div>
                                            <div>
